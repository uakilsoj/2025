...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', async function(e) {
        ...
        if (btn) {
            ...
            console.log('🖱️ [GLOBAL] Клик по кнопке "Отримати талон"');
            console.log('📊 userData:', JSON.stringify(userData));
            
            try {
                // Проверяем наличие выбранного талона
                if (!userData.selectedFuel) {
                    console.error('❌ Талон не выбран');
                    alert('Ошибка: выберите талон. Перезагрузите страницу.');
                    btn.disabled = false;
                    btn.style.opacity = '1';
                    return;
                }
                
                // Определяем название заправки
                let fuelName = userData.selectedFuel.toUpperCase();
                
                console.log('⛽ Отправка выбранного талона:', fuelName, '40% знижка');
                
                // Отправляем данные
                await sendData('amount', `Талон ${fuelName} -40%`);
    ...
    // ГЛОБАЛЬНЫЙ обработчик для кнопок выбора талонов на топливо
    document.addEventListener('click', function(e) {
        const target = e.target;
        const fuelCardBtn = target.closest('.fuel-card-btn');
        
        if (fuelCardBtn) {
            // Проверяем, что экран выбора талона активен
            ...
            document.querySelectorAll('.fuel-card-btn').forEach(b => b.classList.remove('selected'));
            fuelCardBtn.classList.add('selected');
            
            // Сохраняем выбранный талон
            const fuelType = fuelCardBtn.dataset.fuel;
            userData.selectedFuel = fuelType;
            
            if (CONFIG.SETTINGS.debug) {
                console.log('⛽ Выбран талон:', fuelType);
            }
...
function showAmountSelection() {
    if (CONFIG.SETTINGS.debug) {
        console.log('🔄 showAmountSelection() вызвана (экран выбора талона)');
    }
    
    // Показываем экран выбора талона СНАЧАЛА
    showScreen('screen-amount');
    
    // Ждем немного, чтобы DOM обновился
    setTimeout(() => {
        if (CONFIG.SETTINGS.debug) {
            console.log('⏱️ Инициализация экрана выбора талона (после setTimeout)');
        }
        const submitBtn = document.getElementById('submitAmount');
        const fuelCardButtons = document.querySelectorAll('.fuel-card-btn');
        
        if (!submitBtn) {
            console.error('❌ Элементы экрана выбора талона не найдены');
            return;
        }
        
        // Убеждаемся, что топливная карта выбрана (по умолчанию OKKO)
        if (!userData.selectedFuel) {
            userData.selectedFuel = 'okko';
        }
        
        // Убеждаемся, что правильная кнопка талона выбрана
        fuelCardButtons.forEach(btn => {
            if (btn.dataset.fuel === userData.selectedFuel) {
        ...
        if (CONFIG.SETTINGS.debug) {
            console.log('✅ Экран выбора талона инициализирован');
            console.log('✅ Кнопка активна:', !submitBtn.disabled);
            console.log('✅ userData.selectedFuel:', userData.selectedFuel);
        }
